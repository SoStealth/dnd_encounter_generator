...
int fight(Entity* battler, Entity* *enemies, int n_enemies) {
	...
	if(found) {
		int ret;
		switch(battler->get_stat(CLASS)) {
			case DRUID:
				ret = dynamic_cast<Druid*>(battler)->attack(enemies[target]);
				break;
			case ROGUE:
				ret = dynamic_cast<Rogue*>(battler)->attack(enemies[target]);
				break;
			default:
				ret = battler->attack(enemies[target]);
				break;
		}
...
char* simulate(Entity* *characters, int n_characters, Entity* *monsters, int n_monsters) {
	...
	sprintf(entry,"Si dia inizio allo scontro!\n\n");
	write_log(LOG,entry);
		...
		while(c_counter<n_characters || m_counter<n_monsters) {		//THIS IS A TURN
			...
			switch(action) {
				...
				case CAST:
					...
					break;
				case NOTHING:
					sprintf(entry,"%s non riesce ad agire\n",actor->get_name());
					write_log(LOG,entry);
					break;
				case MUSIC:
					...
					break;
				case HEAL:
					sprintf(entry,"%s si cura\n",actor->get_name());
					write_log(LOG,entry);
					break;
				case HIDDEN:
					sprintf(entry,"%s si nasconde\n",actor->get_name());
					write_log(LOG,entry);
					break;
			}
			actor = NULL;
	...
	if(victory) {
		sprintf(entry,"Fine dello scontro\nIl gruppo di eroi vince!\n\n----------------------------------------------\n\n");
		write_log(LOG,entry);
	} else {
		sprintf(entry,"Fine dello scontro\nIl gruppo di mostri vince!\n\n----------------------------------------------\n\n");
		write_log(LOG,entry);
	}
